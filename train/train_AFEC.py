...
def pre_train(model, data_loader, optimizer, epoch, device, config, iteration, lr_schedule):
    global Eiters
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.6f}'))
    metric_logger.add_meter('loss', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    metric_logger.add_meter('loss_ita', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    metric_logger.add_meter('loss_mlm', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    header = 'Forward Epoch: [{}]'.format(epoch)
    print_freq = config['window_size']
    torch.cuda.empty_cache()  
    model.train()  
    data_loader.sampler.set_epoch(epoch)
...
def update_train(model, data_loader, optimizer, epoch, device, config, iteration, lr_schedule, older_params, old_fisher, forward_params, forward_fisher):
    global Eiters
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.6f}'))
    metric_logger.add_meter('loss', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    metric_logger.add_meter('loss_ita', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    metric_logger.add_meter('loss_mlm', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    metric_logger.add_meter('loss_AFEC', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    header = 'Train Epoch: [{}]'.format(epoch)
    print_freq = config['window_size']
    torch.cuda.empty_cache()  
    model.train()  
    ...
    for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_AFEC = compute_AFEC(model, old_fisher, older_params, forward_fisher=forward_fisher, forward_params=forward_params)
        loss =  loss + loss_AFEC
        ...
        metric_logger.update(loss_ita=loss_ita.item())
        metric_logger.update(loss_mlm=loss_mlm.item())
        metric_logger.update(loss_AFEC=loss_AFEC.item())
        metric_logger.update(loss=loss.item())
        metric_logger.update(lr=optimizer.param_groups[0]["lr"])

        tb_logger.log_value('loss_ita', loss_ita.item(), step=Eiters)
        tb_logger.log_value('loss_mlm', loss_mlm.item(), step=Eiters)
        tb_logger.log_value('loss_AFEC', loss_AFEC.item(), step=Eiters)
        tb_logger.log_value('loss', loss.item(), step=Eiters)
        Eiters+=1

        torch.cuda.empty_cache()  
        del id, image, caption, loss, loss_ita, loss_mlm, loss_AFEC
...
def compute_AFEC(model, old_fisher, older_params, forward_fisher, forward_params):
    loss_reg = 0
    lamb = 5000
    lamb_forward = 1
    for n, p in model.module.named_parameters():
        if n in old_fisher.keys():
            loss_reg += (lamb * (torch.sum( old_fisher[n] * (p - older_params[n]).pow(2))/ 2) 
                + lamb_forward * (torch.sum(forward_fisher[n] * (p - forward_params[n]).pow(2))/ 2))
    return loss_reg
...
def getFisherDiagonal(model_ddp, data_loader, device):
    ...
    for n, p in fisher.items():
        fisher[n] = p / len(data_loader)
    return fisher


def main_AFEC(args,config,industry_id_label, all_id_info, device): 
    if 1:
    ...
    fisher = {n: torch.zeros(p.shape).to(device) for n, p in model.module.named_parameters() if p.requires_grad}
   ...
   #####task_list########
    ...
    for iteration , task_i in enumerate(config['task']):
        ...
        if iteration==0:
            ...
            forward_state_dict = copy.deepcopy(model_without_ddp.state_dict())
            ...
            forward_optimizer = torch.optim.AdamW([{'params': model_params[key], 'lr':config['init_lr']} for key in model_params.keys()],weight_decay=config['weight_decay'])

            print(f'now init_lr {a_init_lr}, now init_lr {b_min_lr}')
            lr_schedule = utils.cosine_scheduler(init_lr = a_init_lr, min_lr = b_min_lr, epochs =config['max_epoch'], niter_per_ep = len(data_loader))
            for epoch in range(0, config['max_epoch']):
                #train forward model
                model.module.load_state_dict(forward_state_dict)
                forward_stats = pre_train(model,
                                        data_loader, 
                                        forward_optimizer, 
                                        epoch, 
                                        device,
                                        ...
                                        iteration,
                                        lr_schedule)
                forward_state_dict = copy.deepcopy(model.module.state_dict())
                forward_fisher = getFisherDiagonal(model, data_loader, device)
                forward_params = {n: p.clone().detach() for n, p in model.module.named_parameters() if p.requires_grad}

                model.module.load_state_dict(model_state_dict)
                train_stats = update_train(model, 
                                        data_loader,
                                        optimizer, 
                                        epoch, 
                                        device, 
                                        ...
                                        iteration, 
                                        lr_schedule, 
                                        older_params=older_params, 
                                        old_fisher=fisher, 
                                        forward_params=forward_params,
                                        forward_fisher=forward_fisher)
                model_state_dict = copy.deepcopy(model.module.state_dict())
                score_test_i2t, score_test_t2i = evaluation(model_without_ddp, test_loader, device, args, config)
                if utils.is_main_process():  
                    test_result = itm_eval(score_test_i2t, score_test_t2i, test_loader.dataset.txt2img, test_loader.dataset.img2txt) 
                    print(test_result)
...
if __name__ == '__main__':
    ...
    main_AFEC(args,config,industry_id_label, all_id_info, device)
