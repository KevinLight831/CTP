...
def init_train(model, data_loader, optimizer, epoch, device, config, iteration, lr_schedule, momentum_model):
    ...
    for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        mode = 'CTP_init'
        loss_ita, loss_mlm= model.forward(mode, image, caption, iteration, epoch, momentum_model=momentum_model)  
...
def momentum_update(model_pairs):
    ...
    for model_pair in model_pairs:           
        for param, param_m in zip(model_pair[0].parameters(), model_pair[1].parameters()):
            param.data = param.data * momentum + param_m.data * (1. - momentum)
