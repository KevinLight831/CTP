...
def update_train(args, model, data_loader, optimizer, epoch, device, config, iteration, lr_schedule, older_params, fisher):
    ...
    metric_logger.add_meter('loss_ita', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    metric_logger.add_meter('loss_mlm', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    metric_logger.add_meter('loss_MAS', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    header = 'Train Epoch: [{}]'.format(epoch)
    print_freq = config['window_size']
    torch.cuda.empty_cache()  
    model.train()  
    ...
    for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        lamda = 1
        loss_MAS = compute_MAS(args, model, fisher, older_params)*lamda
        loss =  loss + loss_MAS
...
def compute_MAS(args, model, fisher, older_params):
    loss = 0
    if args.distributed:
...
def get_Memory_Aware_Synapses(model_ddp, data_loader, device):
    ...
    for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq=config['window_size'], header= 'EWC Epoch: [{}]'.format(0))):
        ...
        id, image, caption = batch
        image = image.to(device,non_blocking=True)
        image_feat, text_feat, fusion_out = model_ddp.module.get_raw_VL_feature(image, caption)
        loss = torch.norm(image_feat, p=2, dim=1).mean() + torch.norm(text_feat, p=2, dim=1).mean() + torch.norm(fusion_out, p=2, dim=1).mean()
        loss.backward()
        for n, p in model.named_parameters():
            if p.grad is not None:
                importance[n] += p.grad.abs().clone()
    for n, p in importance.items():
        importance[n] = p / len(data_loader)
    return importance 

def main_MAS(args,config,industry_id_label, all_id_info, device): 
    if 1:
   ...
   #####task_list########
    ...
    for iteration , task_i in enumerate(config['task']):
        ...
        if iteration==0:
            ...
            if args.distributed:
                fisher = get_Memory_Aware_Synapses(model, data_loader, device)
            ...
            if args.distributed:
                new_fisher = get_Memory_Aware_Synapses(model, data_loader, device)
...
if __name__ == '__main__':
    ...
    main_MAS(args,config,industry_id_label, all_id_info, device)
