...
def compute_RWalk(model, scores,fisher, older_params):
    loss_reg = 0
    lamda = 1000 
    for n, p in model.module.named_parameters():
        if n in scores.keys() and n in fisher.keys():
            loss_reg += torch.sum((scores[n]+fisher[n]) * (p - older_params[n]).pow(2))
    loss = lamda*loss_reg
    # print('%.8f' %loss)
    return loss

def update_train(model, data_loader, optimizer, epoch, device, config, iteration, lr_schedule, older_params=None, scores=None, w=None, fisher=None):
    global Eiters
    ...
    metric_logger.add_meter('loss_RWalk', utils.SmoothedValue(window_size=config['window_size'], fmt='{value:.4f}'))
    ...
    for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if iteration>0:
            ...
            loss_ita, loss_mlm = model.forward(mode, image, caption, iteration, epoch)  
            loss = (loss_ita + loss_mlm)
            loss_RWalk = compute_RWalk(model,scores,fisher,older_params)
            loss += loss_RWalk
            optimizer.zero_grad()
            loss.backward()
        ...
        metric_logger.update(loss_RWalk=loss_RWalk.item())
        ...
        tb_logger.log_value('loss_RWalk', loss_RWalk.item(), step=Eiters)
        tb_logger.log_value('loss', loss.item(), step=Eiters)
        Eiters= Eiters+1

        torch.cuda.empty_cache()  
        del id, image, caption, loss, loss_ita, loss_mlm, loss_RWalk
...
def main_RWalk(args,config,industry_id_label, all_id_info, device): 
    if 1:
    ...
    older_params = {n: p.clone().detach() for n, p in model.module.named_parameters()
                if p.requires_grad}
    fisher = {n: torch.zeros(p.shape).to(device) for n, p in model.module.named_parameters()
                if p.requires_grad}
    scores = {n: torch.zeros(p.shape).to(device) for n, p in model.module.named_parameters()
                       if p.requires_grad}
   ...
   #####task_list########
    ...
    for iteration , task_i in enumerate(config['task']):
        ...
        if iteration==0:
            ...
            lr_schedule = utils.cosine_scheduler(init_lr = a_init_lr, min_lr = b_min_lr, epochs =config['max_epoch'], niter_per_ep = len(data_loader))
            for epoch in range(0, config['max_epoch']):
                train_stats, w = update_train(model, data_loader, optimizer, epoch, device, config, iteration, lr_schedule, older_params=older_params, scores=scores,w=w, fisher=fisher)
                score_test_i2t, score_test_t2i = evaluation(model_without_ddp, test_loader, device, args, config)
                if utils.is_main_process():  
                    test_result = itm_eval(score_test_i2t, score_test_t2i, test_loader.dataset.txt2img, test_loader.dataset.img2txt) 
                    print(test_result)
        ...
        curr_fisher = getFisherDiagonal(model, data_loader, device)
        alpha = iteration/(iteration+1)
        param_num = 0
        fisher_mean = 0
        for n, p in curr_fisher.items():
        ...
        curr_score = {n: torch.zeros(p.shape).to(device) for n, p in model.module.named_parameters()
                    if p.requires_grad}
        damping = 0.1
        with torch.no_grad():
            ...
            for n, p in scores.items():
                curr_score[n] = w[n] / (
                        fisher[n] * ((curr_params[n] - older_params[n]) ** 2) + damping)
                w[n].zero_()
        ...
        for n, p in scores.items():
            scores[n] = (scores[n] + curr_score[n]) / 2
...
if __name__ == '__main__':
    ...
    main_RWalk(args,config,industry_id_label, all_id_info, device)
